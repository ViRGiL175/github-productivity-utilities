...
| [`./.github/workflows/detach-inbox-sub-issues.yml`](./.github/workflows/detach-inbox-sub-issues.yml) | Обрабатывает одну issue при переходе `Horizon` из другого значения в `📥 Inbox`: сохраняет ссылку на бывшего родителя в управляемом блоке описания и снимает связь. Первичное размещение в Inbox и иерархию внутри Inbox не трогает. По умолчанию работает в `dry_run`. |
